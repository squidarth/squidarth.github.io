...
Last week, Lucy Zhang and I worked on compressing images with the
...
In addition to being a major plot element in  *Silicon Valley*, compression is also the process of
reducing the amount of space needed to represent some data. In practice, that "data" could be text,
...
Compression with DCTs is a form of lossy compression--here are the results from the code we
wrote: 
...
applying a Fourier transform would yield a bunch of different `sin` and `cosine`
...
(i,j) in the matrix corresponds to the pixel in position (i,j) in the image, and the value of
that pixel is its intensity. `0` corresponds to black, `255` corresponds to white.

It may not be obvious that any image data is "wave-like", so let's try graphing some image data!

Taking the baboon image from above, let's take a row of pixels from halfway down the image (it's 256 pixels long, so this picture is only the first few):

![pixel_row]({{ "/assets/pixel_row_png.png" | absolute_url }})

Graphing the entire row of pixels, we get a picture that looks like this:
...
And more importantly, from this picture, it's pretty easy to image that similarly to the audio example above, this could also be represented as the sum of some trignometric functions. 
...
that picture into different `sin` and `cosine` waves might be useful in compressing the image.
...
is that the transformation that we use to decompose the data into consine functions must
...
$$X_{i,j}  =  cos(\frac{\pi}{N} * i * (j + \frac{1}{2}))$$_
...
I never really explained why chose to use the DCT. As I alluded to earlier
in this post, there are a few requirements we have for the decomposition function--notably, it
must be invertible.

It so happens that there other functions that have this property, notably, the [Discrete Fourier Transform](https://en.wikipedia.org/wiki/Discrete_Fourier_transform) (DFT). As you'd expect, it's totally
possible to do compression with the DFT as well. The main difference between these transforms
is that the DFT does not just use cosine waves--the waves that it uses are a combination
of sin and cosine. I'm not going to get into the details as to why here, but in practice, the DCT typically
allows for more signal to be retained with fewer functions.

# Final Thoughts

I hope that you too are as blown away by the power of the DCT as me. If you're curious about
