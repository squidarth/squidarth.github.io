...
Let's start with the solutions that are obviously bad. We could, for instance, have put some kind
of timeout on the `read_from_child_process` function call, and move on if the timeout is reached. This is
...
to read from each of them, rather than having to have multiple threads that block on each
client.
